...
const listarCestaService = async (page = 1, statusFiltro = "CRIADA") => {
  try {
    const response = await api.get(`/cestas/listar?statusCesta=${statusFiltro}&page=${page}`);
...
const cancelarCesta = async (idCesta: number) => {
  try {
...
export { gerarCestaService, listarCestaService, cancelarCesta };